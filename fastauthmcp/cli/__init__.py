"""FastAuthMCP CLI - command-line interface for running servers and managing auth."""
...
def run(config_path: str | None, transport: str, host: str, port: int) -> None:
    ...
    try:
        ...
        server = FastAuthMCP(name="fastauthmcp", config=config_path)
...
@cli.command()
def doctor() -> None:
    """Run diagnostics on the FastAuthMCP configuration and connectivity."""
    all_passed = True
...
def config_validate() -> None:
    """Validate the fastauthmcp.yaml configuration file."""
    try:
        loader = ConfigLoader()
