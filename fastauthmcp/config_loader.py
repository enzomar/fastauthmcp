"""Configuration loader for the FastAuthMCP framework.

Handles YAML parsing, environment variable overrides, and configuration
resolution from multiple sources (path argument, FASTAUTHMCP_CONFIG env var,
or CWD fastauthmcp.yaml).
"""
...
class ConfigLoader:
    """Loads and validates FastAuthMCP configuration from YAML files and env vars.

    Configuration Resolution Order:
        1. If `path` argument is provided, use it directly
        2. If FASTAUTHMCP_CONFIG env var is set, use that path
        3. Otherwise, look for fastauthmcp.yaml in CWD
        4. If no config found anywhere, return empty FastAuthMCPConfig (passthrough)
    """
    ...
    def load(self, path: Path | None = None) -> FastAuthMCPConfig:
        ...
        Args:
            path: Explicit path to a fastauthmcp.yaml file. If None, resolution
                  order is applied (env var, then CWD).

        Returns:
            A validated FastAuthMCPConfig instance.

        Raises:
            ConfigurationError: If the config file is missing (when FASTAUTHMCP_CONFIG
    ...
    def apply_env_overrides(self, config: FastAuthMCPConfig) -> FastAuthMCPConfig:
        """Apply FASTAUTHMCP_-prefixed environment variable overrides to config.

        Converts env var names to config dot-paths:
            FASTAUTHMCP_AUTH_PROVIDER → auth.provider
        ...
        Returns:
            A new FastAuthMCPConfig with env var overrides applied.
        """
        config_dict = config.model_dump(mode="json", exclude_none=True)
        overrides_applied = False

        for key, value in os.environ.items():
            if not key.startswith("FASTAUTHMCP_"):
                continue

            # Skip the FASTAUTHMCP_CONFIG env var — it's for path resolution
            if key == "FASTAUTHMCP_CONFIG":
                continue

            # Convert FASTAUTHMCP_AUTH_PROVIDER → ["auth", "provider"]
            parts = key[len("FASTAUTHMCP_") :].lower().split("_")
            dot_path = parts
        ...
        try:
            return FastAuthMCPConfig.model_validate(config_dict)
    ...
    def watch(
        self,
        callback: Callable[[FastAuthMCPConfig], None],
        ...
        Args:
            callback: Function called with the new FastAuthMCPConfig on successful reload.
        ...
        def _poll_loop() -> None:
            while not self._watch_stop_event.is_set():
                ...
                try:
                    merged_config = FastAuthMCPConfig.model_validate(merged_data)
                except ValidationError as exc:
        ...
        self._watch_thread = threading.Thread(
            target=_poll_loop, name="fastauthmcp-config-watcher", daemon=True
        )
    ...
    def _resolve_path(self, path: Path | None) -> Path | None:
        ...
        Raises:
            ConfigurationError: If FASTAUTHMCP_CONFIG is set but file doesn't exist.
        """
        ...
        # 2. FASTAUTHMCP_CONFIG env var
        env_path = os.environ.get("FASTAUTHMCP_CONFIG")
        ...
        # 3. CWD fastauthmcp.yaml
        cwd_path = Path.cwd() / "fastauthmcp.yaml"
        if cwd_path.exists():
            return cwd_path
    ...
    def _validate(self, data: dict[str, Any]) -> FastAuthMCPConfig:
        """Validate parsed YAML data against the FastAuthMCPConfig model.

        Raises:
            ConfigurationError: If validation fails (e.g., unknown top-level keys).
        """
        try:
            return FastAuthMCPConfig.model_validate(data)
        except ValidationError as exc:
    ...
    def _apply_scalar_override(
        ...
        e.g., FASTAUTHMCP_AUTH_CALLBACK_TIMEOUT → try auth.callback_timeout first,
