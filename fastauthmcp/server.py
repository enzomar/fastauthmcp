"""FastAuthMCP - Drop-in replacement for fastmcp.FastMCP with enterprise features."""
...
class _FastAuthMCPBridgeMiddleware(Middleware):
    """FastMCP-native middleware that bridges to the FastAuthMCP pipeline.

    Intercepts tool calls (on_call_tool) and runs the FastAuthMCP middleware
    ...
    def __init__(self, fastauthmcp_server: "FastAuthMCP") -> None:
        self._fastauthmcp = fastauthmcp_server
    ...
    async def on_call_tool(
        ...
        # Build a FastAuthMCP RequestContext for this tool call
        ctx = RequestContext(tool_name=tool_name)

        # Initialize request-scoped context propagation
        init_request_context({"tool_name": tool_name})

        try:
            # The handler at the end of the FastAuthMCP pipeline delegates back to
            # FastMCP's call_next, which actually executes the tool function.
            async def fastmcp_handler() -> Any:
                return await call_next(context)

            # Execute the FastAuthMCP pipeline (auth, authz, observability, etc.)
...
class FastAuthMCP:
    ...
    When no fastauthmcp.yaml is found, operates in passthrough mode — behaving
    identically to a vanilla FastMCP instance with no middleware active.
    """

    def __init__(
        self,
        name: str = "fastauthmcp",
        config: str | Path | None = None,
        **kwargs: Any,
    ) -> None:
        """Initialize FastAuthMCP.

        Args:
            name: Server name (passed to FastMCP).
            config: Path to fastauthmcp.yaml. If None, uses FASTAUTHMCP_CONFIG env var
        ...
        self._config: FastAuthMCPConfig = loader.load(path=config_path)
        ...
        # tool calls are intercepted and routed through the FastAuthMCP pipeline.
        if not self._passthrough:
            self._app.add_middleware(_FastAuthMCPBridgeMiddleware(self))
    ...
    def enable_fastauthmcp(
        ...
        Returns:
            A FastAuthMCP instance wrapping the provided FastMCP app.
        ...
        if loaded_config.plugins:
            plugins = FastAuthMCP._load_plugins_from_refs(loaded_config.plugins)

        # Everything validated — now build the instance
        instance = object.__new__(FastAuthMCP)
        instance._app = app
        instance._config = loaded_config
        instance._passthrough = FastAuthMCP._is_passthrough(loaded_config)
        ...
        if not instance._passthrough:
            instance._app.add_middleware(_FastAuthMCPBridgeMiddleware(instance))
    ...
    @staticmethod
    def _is_passthrough(config: FastAuthMCPConfig) -> bool:
        """Determine if FastAuthMCP should run in passthrough mode.
