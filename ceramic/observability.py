...
logger = logging.getLogger("ceramic.observability")
...
class TelemetryService:
    ...
    def __init__(self, config: ObservabilityConfig | None = None) -> None:
        ...
        self._tracer: Tracer = trace.get_tracer("ceramic")
    ...
    def start_span(self, tool_name: str, request_id: str) -> Any:
        ...
        try:
            span = self._tracer.start_span(
                name=f"ceramic.tool.{tool_name}",
                attributes=self._redactor.redact(
                    {
                        "ceramic.tool_name": tool_name,
                        "ceramic.request_id": request_id,
                    }
                ),
            )
            return span
    ...
    def end_span(self, span: Any, outcome: str, duration_ms: float) -> None:
        ...
        try:
            ...
            attributes = self._redactor.redact(
                {
                    "ceramic.outcome": outcome,
                    "ceramic.duration_ms": duration_ms,
                }
            )
