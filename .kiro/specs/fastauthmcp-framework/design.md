# Design Document: FastAuthMCP Framework

## Overview

FastAuthMCP is a Python framework that wraps FastMCP via internal composition (delegation) to provide enterprise capabilities—authentication, authorization, observability, session management—while preserving 100% API compatibility with FastMCP's public surface. The core architectural principle is that FastAuthMCP never inherits from or reimplements FastMCP; it instantiates a FastMCP instance internally and delegates all MCP protocol handling to it, intercepting the request lifecycle through a middleware pipeline.

The framework is configured declaratively via `fastauthmcp.yaml`, activated by a single import change (`from fastauthmcp import FastMCP`), and operated through a dedicated CLI (`fastauthmcp run/login/logout/whoami/doctor/config validate`). A plugin system allows third-party extensions at well-defined hook points.

### Key Design Decisions

1. **Composition over Inheritance**: FastAuthMCP holds a `FastMCP` instance as a private attribute, forwarding all public API calls. This decouples FastAuthMCP's lifecycle from FastMCP's internal implementation.
...
3. **Configuration-Driven Activation**: Features are opt-in via `fastauthmcp.yaml`. When no config is present, FastAuthMCP behaves identically to vanilla FastMCP.
...
graph TB
    ...
    subgraph "FastAuthMCP Framework"
        FastAuthMCPFastMCP[FastAuthMCPFastMCP<br/>Public API Facade]
        Config[ConfigLoader<br/>fastauthmcp.yaml + env]
    ...
    SDK -->|MCP Request| FastAuthMCPFastMCP
    CLI -->|Commands| FastAuthMCPFastMCP
    FastAuthMCPFastMCP --> Config
    FastAuthMCPFastMCP --> MW
    MW --> AuthN
    AuthN --> AuthZ
    AuthZ --> Obs
    Obs --> Sess
...
sequenceDiagram
    ...
    participant F as FastAuthMCPFastMCP
...
### 1. FastAuthMCPFastMCP (Public Facade)

The drop-in replacement class that mirrors FastMCP's public API.

```python
class FastAuthMCPFastMCP:
    """Drop-in replacement for fastmcp.FastMCP with enterprise features."""
    
    def __init__(self, name: str = "fastauthmcp", config: str | Path | None = None, **kwargs):
        """
        Args:
            name: Server name (passed to FastMCP).
            config: Path to fastauthmcp.yaml. If None, uses CERAMIC_CONFIG env var
                    or ./fastauthmcp.yaml. If no config found, runs in passthrough mode.
    ...
    # FastAuthMCP-specific API
    def use(self, plugin: "MiddlewarePlugin") -> None: ...
    
    # Migration helper for existing FastMCP instances
    @staticmethod
    def enable_fastauthmcp(app: "fastmcp.FastMCP", config: str | Path | None = None) -> "FastAuthMCPFastMCP": ...
```
...
```python
class FastAuthMCPConfig(BaseModel):
    """Pydantic model for fastauthmcp.yaml validation."""
...
class ConfigLoader:
    def load(self, path: Path | None = None) -> FastAuthMCPConfig: ...
    def apply_env_overrides(self, config: FastAuthMCPConfig) -> FastAuthMCPConfig: ...
    def watch(self, callback: Callable[[FastAuthMCPConfig], None]) -> None: ...
```
...
class MiddlewarePlugin(Protocol):
    """Interface for plugins registered via app.use() or fastauthmcp.yaml."""
    name: str
...
```python
# fastauthmcp/cli.py - Click-based CLI
@click.group()
def cli(): ...
...
```python
class FastAuthMCPTestClient:
    """Test client that simulates authenticated requests."""
    
    def __init__(
        self,
        app: FastAuthMCPFastMCP,
        email: str | None = None,
...
### Configuration Schema (fastauthmcp.yaml)

```yaml
# Full fastauthmcp.yaml schema
auth:
  provider: "oidc"                    # Provider type
  issuer: "https://idp.example.com"   # OIDC issuer URL
  client_id: "fastauthmcp-app"            # OAuth2 client ID
...
plugins:
  - module: "fastauthmcp_rate_limit"      # Python module path
    config:
      max_requests: 100
      window_seconds: 60
...
*For any* valid MCP tool, prompt, or resource definition that can be registered on a FastMCP instance, registering the same definition on a FastAuthMCPFastMCP instance (with no fastauthmcp.yaml) should succeed and produce an identical registration (same name, same callable signature, same metadata), and identical MCP protocol requests should produce structurally identical responses.

**Validates: Requirements 1.1, 1.2, 1.3, 1.4**

### Property 2: Configuration Source Resolution

*For any* combination of a fastauthmcp.yaml file in the current working directory and a `CERAMIC_CONFIG` environment variable pointing to an alternative path, the ConfigLoader should load from the env var path when set (regardless of CWD file existence), and from CWD when the env var is unset.

**Validates: Requirements 2.1**

### Property 3: Configuration Section Activates Middleware

*For any* valid FastAuthMCPConfig where a section (auth, observability, authorization, sessions) is present and enabled, the constructed middleware pipeline should contain the corresponding middleware layer; and conversely, when a section is absent, that middleware should not be present.

**Validates: Requirements 2.3, 2.4, 2.5, 2.6**

### Property 4: Environment Variable Override Semantics

*For any* scalar value at dot-path `x.y.z` in FastAuthMCPConfig and an environment variable `CERAMIC_X_Y_Z` with a new value, the resolved config should contain the env var value at that path. *For any* non-scalar (list or object) config path and a corresponding env var, the original config value should be preserved unchanged.

**Validates: Requirements 2.7, 2.9**

### Property 5: Invalid Configuration Rejection

*For any* YAML input containing unknown top-level keys or invalid syntax, the ConfigLoader should raise a validation error and never return a FastAuthMCPConfig instance.
...
*For any* active request context with an authenticated identity, `ctx.identity` and `fastauthmcp.identity()` should return the same IdentityContext object (identity by `is`).
...
*For any* FastAuthMCPTestClient configured with specific identity parameters (email, subject, roles, groups), requests made through the client should carry an IdentityContext matching those parameters, and authorization policies should be enforced identically to production (rejecting when configured roles/groups are insufficient).
...
- **Tag format**: `# Feature: fastauthmcp-framework, Property {N}: {title}`
...
└── conftest.py              # Shared fixtures, FastAuthMCPTestClient setup
```

### Key Testing Patterns

- **Config generation**: Hypothesis strategies for generating valid and invalid `FastAuthMCPConfig` instances
...
- **Request simulation**: FastAuthMCPTestClient with configurable identity for authorized/unauthorized scenarios
