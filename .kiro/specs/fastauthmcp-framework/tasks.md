# Implementation Plan: FastAuthMCP Framework

## Overview

This plan implements the FastAuthMCP framework as a Python package that wraps FastMCP via composition, providing enterprise capabilities (authentication, authorization, observability, session management) activated by a single import change and configured via `fastauthmcp.yaml`. The implementation proceeds bottom-up: core infrastructure first, then layered enterprise features, and finally CLI and testing utilities.

## Tasks

- [x] 1. Project structure and core interfaces
  - [x] 1.1 Set up project structure and package scaffolding
    - Create `fastauthmcp/` package with `__init__.py` exposing `FastMCP`, `require_role`, `require_group`, `identity`
    ...
    - Create directory structure: `fastauthmcp/`, `fastauthmcp/middleware/`, `fastauthmcp/auth/`, `fastauthmcp/cli/`, `fastauthmcp/testing/`, `tests/`, `tests/properties/`, `tests/unit/`, `tests/integration/`
    - _Requirements: 1.1, 1.2_

  - [x] 1.2 Define exception hierarchy and data models
    - Implement `fastauthmcp/exceptions.py` with `FastAuthMCPError`, `ConfigurationError`, `AuthenticationError`, `AuthorizationError`, `ProviderError`, `SessionError`, `PluginError`
    - Implement `fastauthmcp/models.py` with `TokenSet`, `Session`, `OIDCEndpoints`, `LogEntry` dataclasses
    - Implement `fastauthmcp/identity.py` with the frozen `IdentityContext` dataclass and module-level `identity()` function using `contextvars`
    - _Requirements: 4.1, 4.2, 4.3, 4.5, 4.6_

  - [x] 1.3 Define middleware protocol and RequestContext
    - Implement `fastauthmcp/middleware/pipeline.py` with `RequestContext` class, `MiddlewareCallable` protocol, `MiddlewarePlugin` protocol
...
- [x] 2. Configuration system
  - [x] 2.1 Implement Pydantic configuration models
    - Create `fastauthmcp/config.py` with `AuthConfig`, `AuthorizationConfig`, `AuthorizationPolicy`, `ObservabilityConfig`, `SessionsConfig`, `PluginRef`, `HotReloadConfig`, `FastAuthMCPConfig` Pydantic models
  ...
  - [x] 2.2 Implement ConfigLoader with YAML parsing and env overrides
    - Create `fastauthmcp/config_loader.py` with `ConfigLoader` class
    - Implement `load()` method: resolve path from `CERAMIC_CONFIG` env var or CWD `fastauthmcp.yaml`
...
- [x] 4. FastAuthMCPFastMCP facade and API compatibility
  - [x] 4.1 Implement FastAuthMCPFastMCP class with delegation
    - Create `fastauthmcp/server.py` with `FastAuthMCPFastMCP` class
    ...
    - Load `FastAuthMCPConfig` on init; if no config found, operate in passthrough mode (no middleware)
  ...
  - [x] 4.2 Implement plugin registration and middleware-attachment migration style
    ...
    - Implement `FastAuthMCPFastMCP.enable_fastauthmcp(app, config)` static method for wrapping existing FastMCP instances
  ...
  - [x]* 4.4 Write property tests for API compatibility
    - **Property 1: API Compatibility** — any definition registered on FastMCP succeeds identically on FastAuthMCPFastMCP with no config, producing identical responses
...
- [x] 5. Security utilities
  - [x] 5.1 Implement LogRedactor
    - Create `fastauthmcp/security.py` with `LogRedactor` class
  ...
  - [x] 5.2 Implement TLSEnforcer
    - Add `TLSEnforcer` to `fastauthmcp/security.py`
...
- [x] 6. Authentication and token management
  - [x] 6.1 Implement OAuthService with PKCE and OIDC discovery
    - Create `fastauthmcp/auth/oauth.py` with `OAuthService` class
  ...
  - [x] 6.2 Implement platform-native TokenStorage
    - Create `fastauthmcp/auth/token_storage.py` with `TokenStorage` protocol
  ...
  - [x] 6.3 Implement AuthenticationMiddleware
    - Create `fastauthmcp/middleware/authentication.py`
...
- [x] 8. Identity context and authorization
  - [x] 8.1 Implement Identity_Context propagation via contextvars
    ...
    - Implement `fastauthmcp.identity()` module-level function using `contextvars.ContextVar`
    - Raise explicit error when `fastauthmcp.identity()` called outside active request context
  ...
  - [x] 8.2 Implement authorization decorators and AuthorizationMiddleware
    - Create `fastauthmcp/authorization.py` with `require_role()` and `require_group()` decorators
    - Create `fastauthmcp/middleware/authorization.py` with `AuthorizationMiddleware`
  ...
  - [x]* 8.3 Write property tests for identity and authorization
    ...
    - **Property 7: Dual Identity Access Equivalence** — `ctx.identity` and `fastauthmcp.identity()` return same object
...
- [x] 9. Observability
  - [x] 9.1 Implement TelemetryService and ObservabilityMiddleware
    - Create `fastauthmcp/observability.py` with `TelemetryService` class
    - Implement `start_span()`, `end_span()`, `emit_log()`, `record_metric()` methods
    - Create `fastauthmcp/middleware/observability.py` with `ObservabilityMiddleware`
  ...
  - [x] 9.2 Implement Prometheus metrics exporter
    - Create `fastauthmcp/metrics.py` with `MetricsExporter` class
...
- [x] 10. Session management
  - [x] 10.1 Implement SessionStore and InMemorySessionStore
    - Create `fastauthmcp/sessions.py` with `SessionStore` protocol and `InMemorySessionStore`
    - Implement `create()`, `get()`, `update()`, `invalidate()` methods
    - Enforce TTL expiration check on `get()`
    - _Requirements: 7.1, 7.5, 7.6_

  - [x] 10.2 Implement SessionMiddleware
    - Create `fastauthmcp/middleware/session.py` with `SessionMiddleware`
...
- [x] 13. CLI implementation
  - [x] 13.1 Implement CLI commands with Click
    - Create `fastauthmcp/cli/__init__.py` with Click group
    - Implement `fastauthmcp run` — load config, start server, print ready message with host:port
    - Implement `fastauthmcp login` — run OAuth flow, store tokens, print email
    - Implement `fastauthmcp logout` — clear tokens, invalidate session
    - Implement `fastauthmcp whoami` — display email, subject, roles; exit non-zero if no session
    - Implement `fastauthmcp doctor` — check IDP connectivity, token freshness, config validity
    - Implement `fastauthmcp config validate` — parse and report errors/warnings
...
- [x] 14. Testing utilities
  - [x] 14.1 Implement FastAuthMCPTestClient and MockIdentityProvider
    - Create `fastauthmcp/testing/__init__.py` with `FastAuthMCPTestClient` class
...
- [x] 15. Integration wiring and package exports
  - [x] 15.1 Wire all components in `fastauthmcp/__init__.py` and finalize public API
    - Export `FastMCP` (alias for `FastAuthMCPFastMCP`), `require_role`, `require_group`, `identity`, `FastAuthMCPTestClient`
    - Ensure `from fastauthmcp import FastMCP` works as drop-in replacement
    - Register CLI entry point in `pyproject.toml` (`[project.scripts] fastauthmcp = "fastauthmcp.cli:cli"`)
    - _Requirements: 1.1, 1.2, 9.1_

  - [x]* 15.2 Write integration tests
    - Test full import-replacement migration: register tool on `fastauthmcp.FastMCP`, call it, verify response
    - Test middleware-attachment: `enable_fastauthmcp()` on existing FastMCP instance
    - Test end-to-end auth + authz flow with `FastAuthMCPTestClient`
